import {
  ...
  WALLET_CONFIG_APP_URL,
} from '@src/utils/constants/appInfo'
import { useCallback, useEffect, useState } from 'react'

import isNull from 'lodash/isNull'
import isString from 'lodash/isString'
import { BladeConnector, ConnectorStrategy, ErrorCodes, HederaNetwork, WalletError } from '@bladelabs/blade-web3.js'
import { Transaction } from '@hashgraph/sdk'
import useLocalStorage from '@src/utils/hooks/useLocalStorage'

export type BladeAccountId = string

export const BLADE_WALLET_LOCALSTORAGE_VARIABLE_NAME = `${WALLET_CONFIG_NAME ?? 'claimbox'}-BladeWalletData-${
  HEDERA_NETWORK ?? 'no-network'
}`
...
const BLADE_CONNECTOR_INSTANCE_DATA = {
  ...
  url: WALLET_CONFIG_APP_URL,
  icons: [WALLET_CONFIG_ICON_URL],
}

const BLADE_CONNECTOR_LISTENERS_METHODS_NAMES = ['onWalletLocked', 'onSessionDisconnect', 'onSessionExpire'] as const

const useBladeWallet = () => {
  const [bladeConnector, setBladeConnector] = useState<null | BladeConnector>(null)

  const [activeAccountId, setActiveAccountId] = useState<string | undefined>(undefined)
  const [localStorageBladeWalletId, updateLocalStorageBladeWalletId] = useLocalStorage(
  ...
  const checkIfBladeConnectorInstanceExists = useCallback(
    (bladeConnector: null | BladeConnector): bladeConnector is BladeConnector => {
  ...
  const setActiveBladeAccount = useCallback(
    async (accountId: string) => {
      if (checkIfBladeConnectorInstanceExists(bladeConnector)) {
        try {
          ...
          throw new Error(`Account "${accountId}" is not paired. Pair this account with BladeWallet first.`)
        }

        const activeAccount = bladeConnector.getSigner()?.getAccountId()

        if (!activeAccount) {
          throw new Error('No active account detected! Please connect to wallet first.')
        }
    ...
    [bladeConnector, checkIfBladeConnectorInstanceExists, updateLocalStorageBladeWalletId]
  )
  ...
  const createSession = useCallback(
    async (accountId?: string) => {
      if (checkIfBladeConnectorInstanceExists(bladeConnector)) {
        ...
        try {
          pairedAccounts = await bladeConnector.createSession(BLADE_SESSION_PARAMS)
        } catch (e) {
          if (
  ...
  const signAndSendTransaction = useCallback(
    async (transaction: Transaction) => {
      ...
      if (!signer) {
        throw new Error('BladeSigner not available! Please connect to wallet first.')
      }
  ...
  // TODO: bladeWallet is opening twice at every page reload
