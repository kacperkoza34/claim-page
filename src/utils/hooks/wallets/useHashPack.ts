...
import useHashConnectEvents from '@src/utils/hooks/wallets/useHashConnectEvents'
import {
  AccountId,
  Transaction,
  TransactionId,
...
const useHashPack = () => {
  ...
  const appConfig = useMemo<HashConnectTypes.AppMetadata>(
    () => ({
      ...
      icon:
        WALLET_CONFIG_ICON_URL ??
        `${window.location.protocol}//${window.location.host}/logo.svg`,
    }),
    [hederaNetworkPrefix]
  )
  ...
  const initializeHashConnect = useCallback(async () => {
    const hashConnectInitData = await hashConnect.init(
      appConfig,
      HEDERA_NETWORK,
      true
    )
  ...
  const populateTransaction = useCallback(
    (tx: Transaction, accountToSign: string) => {
  ...
  //SEND TRANSACTION
  const sendTransactionWithHashPack = useCallback(
    async (tx: Transaction) => {
      ...
      tx = populateTransaction(tx, hashConnectState.pairingData.accountIds[0])
